...
Only a repository administrator can create, move, or delete a `v*` tag. The rule is the
ruleset in @ref[.github/rulesets/release-tags.json], applied the same way as the branch ruleset
(@[Testing] §10); it means the release workflow's `contents: write` token, or any other
workflow, cannot start a release. Branch protection for `main` is in @[Testing] §10.

## 2. What runs

@[ReleaseWorkflow] is generated by cargo-dist from @[DistConfig] and runs on any tag that looks
like a version, and on nothing else: `pr-run-mode = "skip"` keeps it off pull requests, and the
stale-workflow check it would have run there is the `dist plan` step of CI's `generated` job
(@[Testing] §9). Jobs, in order:
...
| `build-local-artifacts` | one runner per target, pinned in @[DistConfig] to current runner images. The Intel macOS archive is cross-compiled on the Apple Silicon runner, which was ninety seconds faster; the binary is executed on a real Intel runner by `verify` below before anything is published |
...
- Any change to @[DistConfig] needs `dist generate` to refresh @[ReleaseWorkflow], or the
  `dist plan` step in CI fails its stale-CI check. Do not hand-edit the generated workflow. CI
  installs whatever `cargo-dist-version` names, so bumping dist is that line plus `dist generate`.
